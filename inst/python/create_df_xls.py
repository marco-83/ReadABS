...
def main_xls(excel_workbook, allowed_blank_rows=1, spreadsheet_type="other"):#, filter_tabs=True):

    # if filter_tabs == "True":
    #     filter_tabs = True
    # elif filter_tabs == "False":
    #     filter_tabs = False
    xl_workbook = xlrd.open_workbook(filename=excel_workbook, formatting_info=True)
    ...
    # try:
    #     results = create_dataframes(tables, xl_workbook, spreadsheet_type)
    #     result_info = table_info(tables)
    # except KeyError:
    #     return pd.DataFrame(), pd.DataFrame()
    #results, tables = create_dataframes(tables, xl_workbook, spreadsheet_type)
...
def create_dataframes(tables, xl_workbook, spreadsheet_type):
    ...
    for i, table in enumerate(tables):
        try:
            ...
            t += 1
            print("Removed table " + str(i) + " because it could not be defined. Possibly a pivot table.")
        else:
            t += 1
...
def pivot_table(column_headings, column_subheadings, xl_workbook, table, spreadsheet_type, df):
    ...
    if table.table_type == "long format":
        #df.columns = reversed(column_headings)
